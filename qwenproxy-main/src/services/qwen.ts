...
async function createRealQwenChat(header: Record<string, string>, accountId?: string): Promise<string> {
  ...
  if (!page || pageClosed || !isOnQwenOrigin) {
    console.warn(`[WarmPool] Skipping browserFetch for ${accountId}: page=${!!page}, closed=${pageClosed}, url=${pageUrl || '(empty)'}`);
  }
  ...
  if (page && !page.isClosed() && isOnQwenOrigin) {
    try {
      ...
      if (result.contentType?.includes('text/event-stream') || result.body?.startsWith('data: ')) {
        ...
        for (const line of lines) {
          ...
          if (trimmed.startsWith('data: ') && trimmed !== 'data: [DONE]') {
            ...
            try {
              ...
              if (parsed && (parsed.chat_id || parsed.id)) {
                parsedBody = jsonStr;
                break;
              }
            } catch {}
  ...
  try {
    ...
    const cause = fetchErr.cause ? ` (cause: ${fetchErr.cause})` : '';
    throw new Error(`Node.js fetch failed for ${accountId}: ${fetchErr.message}${cause}`);
  }
  ...
  if (trimmedBody.startsWith('data: ') || trimmedBody.startsWith('data:[')) {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith('data: ') && trimmed !== 'data: [DONE]') {
        ...
        try {
          const parsed = JSON.parse(jsonStr);
          if (parsed && (parsed.chat_id || parsed.id || parsed.success !== undefined)) {
            json = parsed;
            break;
          }
        } catch {}
    ...
    if (!json) {
      throw new Error(`Failed to parse SSE response from Qwen: ${bodyText.slice(0, 500)}`);
    }
  } else {
...
async function fetchUnusedChats(headers: Record<string, string>, accountId?: string): Promise<string[]> {
  ...
  try {
    ...
    const json = JSON.parse(bodyToParse);
...
export async function createQwenStream(
  ...
  console.log('[QwenStream] createQwenStream called with accountId:', accountId);
  let chatId: string;
  ...
  if (accountId === 'guest') {
    ...
    try {
      console.log('[QwenStream] Retrieving warmed chat for account:', accountId || 'global');
      chatEntry = await getWarmedChat(accountId);
      console.log('[QwenStream] Warmed chat retrieved. chatId:', chatEntry.chatId);
    } catch (err: any) {
  ...
  const sessionKey = accountId || 'global';
  const actualParentId: string | null = getSessionParent(sessionKey) ?? null;
  ...
  if (page && !page.isClosed() && page.url().includes('chat.qwen.ai')) {
    try {
      console.log('[QwenStream] Calling browserStreamFetch for url:', url);
      ...
      console.log('[QwenStream] browserStreamFetch returned. status:', browserResult.status, 'contentType:', browserResult.contentType);
  ...
  console.log('[QwenStream] Falling back to Node.js fetch for url:', url);
  const controller = new AbortController();
  ...
  clearTimeout(timeoutId);
  console.log('[QwenStream] Node.js fetch completed. status:', response.status, 'ok:', response.ok);
  ...
  if (response.ok && !responseContentType.includes('text/event-stream') && response.body) {
    ...
    if (peekText.trimStart().startsWith('data: ')) {
