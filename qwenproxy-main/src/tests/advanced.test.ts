  ...
  globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (urlStr.includes('chat.qwen.ai')) {
      ...
      if (urlStr.includes('/api/v2/chats/new')) {
        return new Response(JSON.stringify({ success: true, chat_id: 'mock-chat-' + Date.now(), id: 'mock-chat-' + Date.now() }), { status: 200, headers: { 'content-type': 'application/json' } });
      }
      if (urlStr.includes('/api/v2/chats/') && !urlStr.includes('completions')) {
        return new Response(JSON.stringify({ success: true, data: [] }), { status: 200, headers: { 'content-type': 'application/json' } });
      }
      if (urlStr.includes('/api/v2/users/user/settings/update')) {
