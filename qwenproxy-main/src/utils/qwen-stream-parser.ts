...
export class QwenStreamParser {
  ...
  constructor(uiSessionId: string, options: QwenStreamParseOptions = {}, accountId?: string) {
  ...
  private updateResponseId(chunk: QwenStreamChunk): void {
    if (chunk['response.created'] && chunk['response.created'].response_id) {
      ...
      updateSessionParent(this.accountId || this.uiSessionId, chunk['response.created'].response_id);
      ...
      updateSessionParent(this.accountId || this.uiSessionId, chunk.response_id);
